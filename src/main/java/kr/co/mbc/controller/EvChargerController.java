...
public class EvChargerController {
	...
	public String list(Criteria criteria, Model model) {
		...
		List<EvChargingStationEntity> evList = evChargerService.findAll(criteria);
	...
	// API받고 DB에 저장(충전기)
		...
		@ResponseBody
		public String chargerFetchAndSaveData() {
		    try {
		        // API에서 XML 데이터 가져오기
		        String xmlData = evChargerService.fetchData(apiUrl, apikey);

		        // XML 데이터를 데이터베이스에 저장
		        evChargerService.saveChagerEntity(xmlData);
