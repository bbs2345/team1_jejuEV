		...
		<form action="/replies/" method="post" id="replyForm"
			...
			<div>
				<c:if test="${empty userEntity}">
					<input type="text" id="replyWriter" class="form-control" placeholder="작성자" required/>
				</c:if>
				<c:if test="${not empty userEntity}">
					<input type="text" id="replyWriter" class="form-control" readonly="readonly" value="${userEntity.username}" />
				</c:if>
			</div>
			...
			<button id="board_read_reply_insert" type="button" class="btn btn-primary">댓글 작성</button>
		</form>
	</div>
